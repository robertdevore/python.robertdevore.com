...
description: "Learn about the complete Python development course and the Kujo SSG and SiteKit stack behind it."
custom_url: about
template: page
order: 20
nav_title: About
...
This course was created by [**Robert DeVore**](<https://robertdevore.com/>), an experienced developer passionate about making Python programming accessible to everyone. Robert brings years of practical development experience to create content that bridges the gap between academic concepts and real-world application.

## Built with Kujo SSG and SiteKit

This site is built with [**Kujo SSG**](<https://github.com/kujolang/ssg>) and [**SiteKit**](<https://github.com/kujolang/site-kit>). Kujo turns the Markdown course source into a fast, deterministic static site, while SiteKit provides the accessible design-system foundation. The theme preserves the original Python blue and yellow palette and uses the locally hosted Departure Mono typeface.
...
This educational platform is built with a small, local-first static stack:

  * **Kujo SSG** : Deterministic static site generation
  * **Markdown** : Clean, readable content format
  * **SiteKit** : Accessible design tokens, components, and Departure Mono
  * **Semantic templates** : Responsive course, lesson, search, and navigation layouts
  * **Optimized Performance** : Fast loading times for better learning experience

## About the Site Architecture

The current architecture keeps the course portable, reviewable, and easy to maintain:

  * Markdown lessons with YAML front matter
  * A complete, vendored SiteKit consumer distribution
  * Locally hosted fonts, scripts, and search data
  * Automatic sitemap, RSS, robots, SEO, and `llms.txt` output
  * A validated build that can deploy to any static hosting platform

The generated site has no database and no required client-side framework. Course pages remain fast, indexable, and readable even when JavaScript is unavailable.
